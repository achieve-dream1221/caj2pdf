    ...
    def __init__(self, data, old_style=False):
        ...
        def Text(self, code):
            try:
                self.characters.append(
                    bytes(
                        [self.data[self.offset + 5], self.data[self.offset + 4]]
                    ).decode("gbk")
                )
        ...
        def TextMulti(self, code):
            ...
            while 1:
                ...
                try:
                    self.characters.append(
                        bytes(
                            [self.data[self.offset + 3], self.data[self.offset + 2]]
                        ).decode("gbk")
                    )
                except UnicodeDecodeError:
                    self.characters.append(
        ...
        def Figure(self, code):
            try:
                ...
                return
            (ignore1, offset_x, offset_y, size_x, size_y, int2, int3, int4, int5) = (
                struct.unpack("<HHHHHIIII", self.data[self.offset : self.offset + 26])
            )
