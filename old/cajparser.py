...
class CAJParser(object):
    ...
    def _convert_hn(self, dest):
        ...
        for i in range(self.page_num):
            ...
            for j in range(images_per_page):
                ...
                read32 = caj.read(32)
                [image_type_enum, offset_to_image_data, size_of_image_data] = (
                ...
                if image_type[image_type_enum] == "JBIG":
                    ...
                    (SOFn, frame_length, bits_per_pixel, height, width, component) = (
    ...
    def _convert_kdh(self, dest):
        ...
        # Use mutool to repair xref
        try:
            check_output(["pymupdf", "clean", dest + ".tmp", dest], stderr=STDOUT)
        except CalledProcessError as e:
            print(e.output.decode("utf-8"))
            raise SystemExit(
                "Command mutool returned non-zero exit status " + str(e.returncode)
